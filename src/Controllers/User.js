...
export const updateUser = async (req, res) => {
  try {
    ...
    const userUpdated = await User.findByIdAndUpdate(
      id,
      { ...req.body, roleNumber: newRole.roleNumber },
      {
        new: true,
      }
    );
...
export const signup = async (req, res) => {
  try {
    ...
    const user = await User.create({
      ...
      roleNumber: roleIsActive.roleNumber,
    });
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    if (!email) {
      return res.status(400).json({
        success: false,
        message: "You haven't provided an Email address!",
      });
    }
    ...
    if (!user) {
      return res
        .status(404)
        .json({ success: false, message: "Địa chỉ email không chính xác!" });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Verify token sent SUCCEED!" });
  } catch (error) {
    return res.status(500).json({
      message: error.message,
    });
...
export const resetPassword = async (req, res) => {
  ...
  try {
    const user = await User.findOne({ "verifyToken.token": otp });
    const hashedPassword = await bcrypt.hash(newPassword, 10);
    ...
    return res
      .status(200)
      .json({ message: "Thay đổi mật khẩu thành công.", success: true });
  } catch (error) {
    return res.status(500).json({
      message: error.message,
    });
...
export const verifyOTPForgotPassword = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Xác thực OTP thành công", otp });
  } catch (error) {
    return res.status(500).json({
      message: error.message,
    });
