...
export const getAllCustomers = async (req, res) => {
  try {
    ...
    if (_createdAt) {
      const startDate = new Date(_createdAt);
      const endDate = new Date(moment(_createdAt).add(1, "days").format());
      query.createdAt = {
        $gte: startDate,
        $lte: endDate,
      };
    ...
    if (existingCustomer) {
      if (existingCustomer.citizenId === req.body.citizenId) {
        return res.status(409).json({
          message: "Căn cước công dân đã bị trùng lặp!",
        });
      }
      if (existingCustomer.phone === req.body.phone) {
        return res.status(409).json({
          message: "Số điện thoại đã bị trùng lặp!",
        });
      }
